...
def confirm_user_account(request, uidb64):
	...
	if valid_link == True:
		...
		end_user.save()
	return render_to_response("registration/user_confirm_done.html",{"validlink":valid_link},context_instance=RequestContext(request))
...
def signup(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			form.save()
			return render_to_response("registration/signup_done.html",context_instance=RequestContext(request))
